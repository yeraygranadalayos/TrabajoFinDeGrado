# -*- coding: utf-8 -*-
"""
Created on Sun Oct 18 19:16:35 2020
...
def load_dataset(filename, cols):
    ...
    df = pd.read_csv(filename, encoding='latin-1', sep=',',engine='python', skiprows=750000, nrows=300000, names=header_names)
    df.columns = cols
    return df
...
def train_random_forest_classifier(features, label):
    print ("Training the random forest classifier model...")
    from sklearn.ensemble import RandomForestClassifier
    forest = RandomForestClassifier(n_estimators = 10)
    forest.fit(features, label)
    print ('Finished random forest classifier model')
    return forest


forest = train_random_forest_classifier(tfidf_features, y_train)
...
predicted_y = forest.predict(test_data_tfidf_features)
